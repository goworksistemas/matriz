  ...
  const columns = useMemo<ColumnDef<ResumoVendedor>[]>(
    () => [
      ...
      },
      {
        ...
        header: 'Negócios',
        cell: ({ row }) => (
          <span className="text-gray-600 dark:text-gray-300">{row.original.comissoes.length}</span>
        ),
        size: 80,
      },
      {
        ...
        header: 'Pos. Calc.',
        cell: ({ getValue }) => (
          <span className="text-gray-600 dark:text-gray-300">{formatNumber(getValue() as number, 2)}</span>
        ),
        size: 100,
      },
      {
        ...
        header: 'Peso',
        cell: ({ getValue }) => (
          <span className="text-gray-600 dark:text-gray-300">{formatNumber(getValue() as number)}</span>
        ),
        size: 80,
      },
      {
    ...
    <div className="space-y-6">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <div className="space-y-2">
              <h4 className="font-semibold text-gray-700 dark:text-gray-200 flex items-center gap-1.5">
                <span className="h-2 w-2 rounded-full bg-green-500"></span>
                Produto Físico (por Posições Calculadas)
              </h4>
              <ul className="space-y-1 text-gray-500 dark:text-gray-400 ml-3.5">
                <li>≥ 65 posições → <span className="text-green-600 dark:text-green-400">35%</span> sobre comissão</li>
                <li>≥ 45 posições → <span className="text-green-600 dark:text-green-400">30%</span> sobre comissão</li>
                <li>≥ 35 posições → <span className="text-green-600 dark:text-green-400">25%</span> sobre comissão</li>
              </ul>
            </div>
            <div className="space-y-2">
              <h4 className="font-semibold text-gray-700 dark:text-gray-200 flex items-center gap-1.5">
              ...
              <ul className="space-y-1 text-gray-500 dark:text-gray-400 ml-3.5">
                <li>≥ 55 → <span className="text-primary-600 dark:text-primary-400">45%</span> sobre comissão</li>
                <li>≥ 45 → <span className="text-primary-600 dark:text-primary-400">35%</span> sobre comissão</li>
                <li>≥ 30 → <span className="text-primary-600 dark:text-primary-400">25%</span> sobre comissão</li>
              </ul>
            </div>
          </div>
        </CardContent>
      ...
      <Card className="!p-4">
        <div className="flex flex-wrap items-end gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-medium text-gray-500 dark:text-gray-400">Venda Impacto</label>
            <Select
              value={
                filtrosVendedor.vendaImpacto === null
                  ? ''
